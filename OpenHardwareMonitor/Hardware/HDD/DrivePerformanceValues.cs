...
{
    public DrivePerformanceValues(WindowsSmart.DiskPerformance rawData)
    {
        BytesRead = rawData.BytesRead;
        BytesWritten = rawData.BytesWritten;
        ReadTime = TimeSpan.FromTicks(rawData
            .ReadTime); // Value is given in 100-nanosecond intervals, which is just the same as the timespan resolution
    ...
    public uint SplitCount { get; }

    public DateTime QueryTime { get; }

    public uint StorageDeviceNumber { get; }

    public string StorageManagerName { get; }
}
